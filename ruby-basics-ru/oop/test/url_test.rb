...
class UrlTest < Minitest::Test
  ...
  def test_equals?
    url = Url.new @yandex_url
    second_url = Url.new 'http://yandex.ru?key2=value2&key=value'
    third_url = Url.new 'http://yandex.ru?key2=value2&key=value&key3=value3'

    assert { second_url.equality?(url) }
    refute { third_url.equality?(url) }
  end
